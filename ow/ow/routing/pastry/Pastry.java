...
//System.out.println("rowSetWidth:     " + rowSetWidth);
			...
			Message reqMsg = PastryMessageFactory.getUpdateRoutingTableMessage(selfIDAddress,
...
//nodes.length + ", " + nLeafSet + ", " + isFinalHop);
	...
	private void requestLeafSet(IDAddressPair target) {
		...
		IDAddressPair[] leafSetArray = (IDAddressPair[])contents[0];
	...
	protected void prepareHandlers() {
		...
		handler = new MessageHandler() {
			public Message process(final Message msg) {
				...
				IDAddressPair[] leafSetArray = (IDAddressPair[])contents[1];
				boolean fromFinalHop = (Boolean)contents[2];
				...
				routingTable.merge(msg.getSource());

				if (leafSet != null && leafSetArray != null) {
					// merge leaf set
					leafSet.merge(msg.getSource());	// source node should be a neighbor
		...
		runtime.addMessageHandler(Tag.JOINED.getNumber(), handler);
	...
	private void sendJoinedMessage(Collection<IDAddressPair> c) {
		...
		Message reqMsg = PastryMessageFactory.getJoinedMessage(selfIDAddress);
