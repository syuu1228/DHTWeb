...
//		}	// if (queryConcurrency <= 1)
		...
		for (int i = 0; i < target.length; i++) {
			...
			if (msgType == Tag.ITE_ROUTE_INVOKE) {
				Serializable res =
					invokeCallbacks(target[i], callbackTag, callbackArgs[i], filter, null, isFinalNode);
						...
						for (int i = 0; i < forkedTarget.length; i++) {
							if (forkedRootCandidates[i] != null) {
								...
								for (IDAddressPair cand: forkedRootCandidates[i]) {
									if (cand != null
											&& !blackList.contains(cand.getAddress())
											&& !(msgType == Tag.ITE_ROUTE_JOIN && getSelfIDAddressPair().equals(cand))) {
										root = cand;
										break;
									}
								}

								if (root != null && !contact.equals(root)) {
									logger.log(Level.WARNING/*INFO*/,
											"Target of the terminating msg is not responsible for "
											+ forkedTarget[i].toString(-1)
											+ "... adjusted from "
											+ contact.getAddress() + " (" + contact.getID().toString(-1) +
											") to "
	...
	private void prepareHandlers() {
		...
		handler = new MessageHandler() {
			public Message process(final Message msg) {
				...
				// routing
				boolean[] isFinalNode = new boolean[target.length];
...
*/
							if (adjustedLastHops[i] != null) {
								isFinalNode[i] = adjustedLastHops[i][0].equals(getSelfIDAddressPair());
							}
						}
					}	// if (routing)
					else {
				...
				if (msg.getTag() == Tag.ITE_ROUTE_INVOKE.getNumber()
					...
					// invoke callbacks
					for (int i = 0; i < target.length; i++) {
						callbackResult[i] = invokeCallbacks(target[i], tag, args[i], filter, lastHop[i], isFinalNode[i]);
