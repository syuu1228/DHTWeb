...
var workingThread = function(threadID) {
...
try {
  ...
  body += "<dl>";
  ["host", "port", "path", "queryString", "httpVersion", "method"].forEach ( function (i) {
    body += '<dt style="font-weight: bold;">' + i + "</dt>" + "<dd>" + metadata[i] + "</dd>";
  } );
  body += "</dl>";
  response.bodyOutputStream.write(body, body.length);
}catch(err){
...
workingThread.prototype = {
  ...
  DEBUG = true;
  var srv = new nsHttpServer();
  srv.registerPathHandler("/bin", pathHandler);
  srv.start(8081);
...
background.dispatch(new workingThread(1), background.DISPATCH_NORMAL);
